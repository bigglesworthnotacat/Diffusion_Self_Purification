#!/usr/bin/env bash
# Finetune model on the purified dataset.
# Prerequisite: merge the adapter weights into the original model first,
# then set LLM_VERSION to the merged checkpoint path.
set -euo pipefail
...
LLM_VERSION=""                            # path to merged model (base + backdoor adapter)
VISION_MODEL_VERSION="model/siglip2-so400m-patch14-384"  # local path or HF model ID
DATA_PATH=""                              # path to purified dataset JSON
IMAGE_FOLDER=""                           # root directory for images
OUTPUT_DIR=""                             # checkpoint output directory

# ---- Config ----
PROMPT_VERSION="llava_llada"
BASE_RUN_NAME="llada_v_finetune_purified"
NUM_EPOCHS=4  # typically 1/5 to 1/10 of the epochs used during backdoor injection
