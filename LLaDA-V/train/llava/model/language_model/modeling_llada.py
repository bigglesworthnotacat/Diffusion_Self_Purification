...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    @torch.no_grad()
    def generate_prompt_mask_kl_approx(
        self,
        inputs_embeds,                          # [1, L_prompt, D]
        *,
        # ====== Single pass: image mask ratio ======
        img_mask_ratio: float,                  # e.g. 0.5

        # ====== Text-side fixed ratio (set to 0.0 to leave text unchanged) ======
        prompt_mask_ratio_text: float = 0.0,

        # ====== Used to locate the target image token span ======
        input_ids: torch.Tensor = None,         # [1, L_ids]
        image_token_index: int = None,
        image_span: tuple | None = None,        # (img_start, img_end) [start, end)
        IMAGE_TOKEN_INDEX: int = None,

        # ====== Generation parameters (consistent with original implementation) ======
        steps: int = 128,
        gen_length: int = 128,
        ...
        # ====== Scoring phase batch size (for position selection only; does not affect generation) ======
        eval_batch_size: int = 64,              # (Not used in Fisher approximation, kept for interface compatibility)

        # ====== KL computed only on top-K tokens (consistent with original implementation) ======
        topk_vocab: int = 10,

        # ====== Combination weights (kept for interface compatibility; only weight_kl is used) ======
        weight_kl: float = 1.0,
        weight_drop: float = 0.0,   # Kept but not used

        # ====== Fisher-Jacobian approximation control hyperparameters ======
        fisher_num_probes: int = 6,         # Number of Hutchinson probes (4~8 commonly used)
        fisher_topk_mode: bool = True,      # True: construct Fisher only in top-K subspace (recommended)
        fisher_score_norm: str = "none",    # {"none","l2","softmax"} normalization applied to scores
        fisher_dtype: str = "float64",      # {"float32","float64"} internal computation precision
        **kwargs
    ):
        """
        Fisher-Jacobian (Hutchinson) approximation for KL-only scoring:
        - Uses m probes q ~ N(0, F) to compute input gradients of s_j = <q, z>,
            estimating 0.5 * (delta_e_i^T J^T F J delta_e_i) for each image token position i.
        - Only replaces the scoring phase; generation logic remains unchanged.
        Returns: cont (torch.LongTensor, [1, L])
        """
        import numpy as np
        import torch
        import torch.nn.functional as F
        ...
        # ====== Utility ======
        def _find_image_span(ids, L_prompt, IMG_IDX):
            ids0 = ids[0]
            orig_len = ids0.shape[0]
            pos = (ids0 == IMG_IDX).nonzero(as_tuple=False)
            if pos.numel() == 0:
                return None
            img_pos0 = int(pos[0].item())
            K_img = max(0, L_prompt - orig_len + int(pos.shape[0]))
            s = max(0, img_pos0)
            e = min(L_prompt, s + K_img)
            return (s, e) if e > s else None

        # ====== Validation ======
        assert 0.0 <= prompt_mask_ratio_text <= 1.0
        r_img = float(img_mask_ratio)
        assert 0.0 <= r_img <= 1.0

        device = inputs_embeds.device
        B, L_prompt, D = inputs_embeds.shape
        assert B == 1
        ...
        # dtype control
        dtype_fisher = torch.float64 if str(fisher_dtype).lower() == "float64" else torch.float32
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if generation_suffix is not None and tokenizer is not None and len(generation_suffix) > 0:
                suffix_token_ids = tokenizer.encode(generation_suffix, add_special_tokens=False)
                if isinstance(suffix_token_ids, list) and len(suffix_token_ids) > 0:
                    ...
                    suffix_token_ids, suffix_embeds, suffix_len = None, None, 0

            # ===== Baseline sequence (prompt unmasked; generation region fully MASKed) =====
            total_length = L_prompt + gen_length + suffix_len
            masked_embed_full = self.model.embed_tokens(torch.tensor([mask_id], device=device))  # [1, D]
            masked_embed = masked_embed_full[0]

            base_x_embeds = masked_embed_full.repeat(1, total_length, 1)
            base_x_embeds[:, :L_prompt] = inputs_embeds.clone()
            if suffix_embeds is not None:
                base_x_embeds[:, -suffix_len:] = suffix_embeds

            base_x_tokens = torch.full((1, total_length), mask_id, dtype=torch.long, device=device)
            if suffix_token_ids is not None:
                base_x_tokens[:, -suffix_len:] = suffix_token_ids

            prompt_index = torch.zeros((1, total_length), dtype=torch.bool, device=device)
            prompt_index[:, :L_prompt] = True

            # ===== Locate image span =====
            txt_mask_prompt = torch.ones(L_prompt, dtype=torch.bool, device=device)
            img_mask_prompt = torch.zeros(L_prompt, dtype=torch.bool, device=device)

            if image_span is not None:
                img_start, img_end = int(image_span[0]), int(image_span[1])
                img_start = max(0, min(img_start, L_prompt))
                img_end   = max(img_start, min(img_end, L_prompt))
            else:
                IMG_IDX = image_token_index if image_token_index is not None else IMAGE_TOKEN_INDEX
                assert IMG_IDX is not None, "Please provide IMAGE_TOKEN_INDEX or image_token_index"
                assert input_ids is not None, "Missing input_ids to infer image span; or pass image_span directly"
                span_tmp = _find_image_span(input_ids, L_prompt, IMG_IDX)
                assert span_tmp is not None, "Failed to locate target image span (check IMAGE_TOKEN_INDEX and template)"
                img_start, img_end = span_tmp

            if img_end > img_start:
                img_mask_prompt[img_start:img_end] = True
                txt_mask_prompt[img_start:img_end] = False
            T_img = int((img_mask_prompt).sum().item())
            assert T_img == (img_end - img_start)

            # ===== Text-side fixed permutation =====
            txt_idx = torch.nonzero(txt_mask_prompt, as_tuple=False).squeeze(-1)
            n_txt = int(txt_idx.numel())
            perm_txt = txt_idx[torch.randperm(n_txt, device=device)] if n_txt > 0 else torch.empty(0, dtype=torch.long, device=device)

            # ===== Fisher-Jacobian (Hutchinson) approximate scoring (replaces original score logic) =====
            gen_T = min(block_length, gen_length)
            eps = 1e-40
            if gen_T == 0 or T_img == 0:
                print("No score")
                img_rank_global = torch.arange(img_start, img_end, device=device, dtype=torch.long)
            else:
                gen_slice = slice(L_prompt, L_prompt + gen_T)

                # ---- Baseline forward: get logits for first block (consistent with original) ----
                with torch.enable_grad():
                    # Enable gradients for this segment (even if outer scope has @no_grad)
                    base_x_embeds = base_x_embeds.detach().clone().requires_grad_(True)

                    base_out = self.get_model()(
                        inputs_embeds=base_x_embeds,
                        output_hidden_states=False,
                        output_attentions=False,
                        return_dict=True,
                    )
                    base_logits = self.lm_head(base_out.last_hidden_state)               # [1, total_length, V]
                    base_logits_slice = base_logits[:, gen_slice, :]                     # [1, G, V]
                    V = base_logits_slice.shape[-1]
                    G = base_logits_slice.shape[1]                                       # = gen_T

                    # ---- Get top-K and p (aligned with original implementation), or full V (optional) ----
                    if fisher_topk_mode:
                        K = max(1, min(int(topk_vocab), int(V)))
                        topk_idx = torch.topk(base_logits_slice, k=K, dim=-1).indices        # [1, G, K]
                        base_sel = torch.gather(base_logits_slice, dim=-1, index=topk_idx)   # [1, G, K]
                        p_sub = F.softmax(base_sel.to(dtype_fisher), dim=-1)                 # [1, G, K]

                        # Helper function to scatter q_sub back to V dimensions
                        def scatter_to_vocab(q_sub):
                            v_full = torch.zeros((1, G, V), device=device, dtype=q_sub.dtype)
                            v_full = v_full.scatter(dim=-1, index=topk_idx, src=q_sub)
                            return v_full
                    else:
                        # Full-vocabulary Fisher (heavier), no top-K subspace
                        p_full = F.softmax(base_logits_slice.to(dtype_fisher), dim=-1)       # [1, G, V]

                    # ---- delta_e (prompt segment only), to be projected with gradients later ----
                    masked_prompt_full_prompt = masked_embed_full.expand(1, L_prompt, D)      # [1, L_prompt, D]
                    delta_e_prompt = (masked_prompt_full_prompt - inputs_embeds).to(dtype_fisher)

                    # ---- Number of Hutchinson probes ----
                    m_probes = max(1, int(fisher_num_probes))

                    # Accumulate energy for each image token
                    score_energy = torch.zeros(T_img, device=device, dtype=dtype_fisher)

                    # Fisher sampling: closed-form implementation of q ~ N(0, F)
                    # F = diag(p) - p p^T  (independent per g)
                    def sample_q_from_fisher(p):  # p: [1, G, K_or_V]
                        y = torch.randn_like(p)                                        # [1, G, K_or_V]
                        u = torch.sqrt(torch.clamp(p, min=eps))                        # [1, G, K_or_V]
                        # Project onto orthogonal complement of u: y_ortho = y - (y·u)u
                        proj = (y * u).sum(dim=-1, keepdim=True) * u
                        y_ortho = y - proj
                        q = u * y_ortho                                                # D^{1/2} * y_ortho
                        return q.to(base_logits_slice.dtype)

                    # Perform m probe backpropagations
                    for j in range(m_probes):
                        if fisher_topk_mode:
                            q_sub = sample_q_from_fisher(p_sub)                         # [1, G, K]
                            v_full = scatter_to_vocab(q_sub)                           # [1, G, V]
                        else:
                            q_full = sample_q_from_fisher(p_full)                       # [1, G, V]
                            v_full = q_full

                        # Scalar s_j = <q, z> = sum_{g,v} q_{g,v} * z_{g,v}
                        s_j = (v_full * base_logits_slice.float()).sum()

                        grad_embeds_j, = torch.autograd.grad(
                            outputs=s_j,
                            inputs=base_x_embeds,
                            retain_graph=(j < m_probes - 1),
                            create_graph=False,
                            allow_unused=False
                        )  # [1, total_length, D]

                        # Keep only prompt segment gradients, project onto image segment
                        g_prompt = grad_embeds_j[:, :L_prompt, :].to(dtype_fisher)      # [1, L_prompt, D]
                        g_img = g_prompt[:, img_start:img_end, :]                       # [1, T_img, D]
                        de_img = delta_e_prompt[:, img_start:img_end, :]                # [1, T_img, D]
                        dot = (g_img * de_img).sum(dim=-1).squeeze(0)                   # [T_img]
                        score_energy += (dot ** 2)

                        # Explicitly release temporary references
                        del v_full, s_j, grad_embeds_j, g_prompt, g_img, de_img, dot

                    # Final score: 0.5 * E[(dot)^2] * weight_kl
                    scores = 0.5 * (score_energy / float(m_probes)) * float(weight_kl)  # [T_img]

                    # Optional normalization
                    if fisher_score_norm == "l2":
                        denom = scores.norm(p=2) + torch.tensor(1e-12, device=device, dtype=dtype_fisher)
                        scores = scores / denom
                    elif fisher_score_norm == "softmax":
                        scores = torch.softmax(scores.to(torch.float32), dim=0).to(dtype_fisher)

                # Higher score = higher priority for masking
                order_local = torch.argsort(scores, descending=True)                     # [T_img]
                img_rank_global = (img_start + order_local).to(torch.long)

                if debug:
                    topk_show = min(10, T_img)
                    tv, ti = torch.topk(scores, k=topk_show, largest=True, sorted=True)
                    print(f"[FISHER-JACOBIAN] probes={m_probes} | topk_mode={fisher_topk_mode} "
                        f"| T_img={T_img} | top-{topk_show} scores:", tv.tolist())
                    print(f"[FISHER-JACOBIAN] top-{topk_show} local idx:", ti.tolist())

            # ===== Sampling grid (generation logic remains unchanged) =====
            assert gen_length % block_length == 0
            num_blocks = gen_length // block_length
            assert steps % num_blocks == 0
            steps_per_block = steps // num_blocks

            # —— stop tokens —— (only when stopping_criteria is non-empty)
            stop_tokens = []
            if stopping_criteria is not None and tokenizer is not None:
                for stop_str in stopping_criteria:
                    ids = tokenizer.encode(stop_str, add_special_tokens=False)
                    if isinstance(ids, list) and len(ids) > 0:
                        stop_tokens.append(ids)

            def forward_and_get_logits(_x_embeds):
                if cfg_scale > 0.:
                    un_embeds = _x_embeds.clone()
                    un_mask = prompt_index.unsqueeze(-1).expand_as(_x_embeds)
                    un_embeds[un_mask] = masked_embed_full.repeat(_x_embeds.shape[0], _x_embeds.shape[1], 1)[un_mask]
                    combined = torch.cat([_x_embeds, un_embeds], dim=0)
                    outputs = self.model(inputs_embeds=combined)
                    logits_all = self.lm_head(outputs[0]).float()
                    logits_c, logits_u = torch.chunk(logits_all, 2, dim=0)
                    return logits_u + (cfg_scale + 1) * (logits_c - logits_u)
                else:
                    outputs = self.model(inputs_embeds=_x_embeds)
                    return self.lm_head(outputs[0]).float()

            def _get_num_transfer_tokens(mask_index, steps_blk):
                mask_num = mask_index.sum(dim=1, keepdim=True)
                base = mask_num // steps_blk
                remainder = mask_num % steps_blk
                num_transfer_tokens = base.expand(-1, steps_blk).clone()
                if remainder.sum() > 0:
                    idx = torch.arange(steps_blk, device=mask_index.device).unsqueeze(0)
                    mk = idx < remainder
                    num_transfer_tokens[mk] += 1
                return num_transfer_tokens.to(torch.int64)

            # ====== Construct prompt mask from single ratio and generate (original logic preserved) ======
            m_txt = int(np.floor(n_txt * prompt_mask_ratio_text)) if n_txt > 0 else 0
            m_img = int(np.floor(T_img * r_img)) if T_img > 0 else 0

            x_embeds = base_x_embeds.clone()
            x_tokens = base_x_tokens.clone()

            prompt_mask_vec = torch.zeros(L_prompt, dtype=torch.bool, device=device)
            if m_txt > 0:
                prompt_mask_vec[perm_txt[:m_txt]] = True
            if m_img > 0 and T_img > 0:
                prompt_mask_vec[img_rank_global[:m_img]] = True

            if prompt_mask_vec.any():
                prompt_mask_3d = prompt_mask_vec.view(1, L_prompt, 1).expand(1, L_prompt, D)
                masked_prompt_full = masked_embed_full.expand(1, L_prompt, D)
                x_embeds[:, :L_prompt, :] = torch.where(prompt_mask_3d, masked_prompt_full, x_embeds[:, :L_prompt, :])
                x_tokens[:, :L_prompt]    = torch.where(
                    prompt_mask_vec.view(1, L_prompt),
                    torch.full_like(x_tokens[:, :L_prompt], mask_id),
                    x_tokens[:, :L_prompt]
                )

            stop_position = L_prompt + gen_length
            found_stop_seq = False
            ...
            for num_block in range(num_blocks):
                ...
                block_embeds = x_embeds[:, block_start:block_end]
                block_mask_index = torch.all(torch.abs(block_embeds - masked_embed_full) < 1e-5, dim=2)
                num_transfer_tokens = _get_num_transfer_tokens(block_mask_index, steps_per_block)

                for i in range(steps_per_block):
                    mask_index_full = torch.all(torch.abs(x_embeds - masked_embed_full) < 1e-5, dim=2)

                    if found_stop_seq:
                        pre_stop_masks = mask_index_full[0, L_prompt:stop_position]
                        if not pre_stop_masks.any():
                            break

                    if not mask_index_full[0, block_start:block_end].any():
                        break

                    logits = forward_and_get_logits(x_embeds)

                    # Suppress certain special tokens (mask positions only)
                    for token_id in [126081, 126080, 126346, 126347]:
                        logits[:, :, token_id] = torch.where(
                            mask_index_full, torch.tensor(-float('inf'), device=device), logits[:, :, token_id]
                        )

                    # Sampling (preserving original behavior)
                    if temperature == 0:
                        logits_noisy = logits
                    else:
                        ...
                        gumbel_noise = (-torch.log(noise)) ** temperature
                        logits_noisy = logits64.exp() / gumbel_noise
                    x0 = torch.argmax(logits_noisy, dim=-1)

                    # Apply predictions only to generation region of current block; prompt/suffix masked out
                    x0_embeds = self.model.embed_tokens(x0)
                    x0_embeds = torch.where(mask_index_full.unsqueeze(-1).expand_as(x_embeds), x0_embeds, x_embeds)
                    x0 = torch.where(mask_index_full, x0, x_tokens)

                    # Select positions to finalize in this step
                    if remasking == 'low_confidence':
                        p64 = F.softmax(logits.to(torch.float64), dim=-1)
                        x0_p = torch.gather(p64, dim=-1, index=x0.unsqueeze(-1)).squeeze(-1)  # [1, T]
                        x0_p_for_select = x0_p.clone()
                    elif remasking == 'random':
                        x0_p_for_select = torch.rand((x0.shape[0], x0.shape[1]), device=x0.device, dtype=torch.float64)
                    else:
                        raise NotImplementedError(remasking)

                    x0_p_for_select[:, block_end:] = -float('inf')
                    x0_p_for_select[:, :L_prompt]  = -float('inf')
                    if suffix_len > 0:
                        x0_p_for_select[:, -suffix_len:] = -float('inf')

                    confidence = torch.where(mask_index_full, x0_p_for_select.to(torch.float32), -float('inf'))
                    transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                    k_this = num_transfer_tokens[0, i]
                    if k_this > 0:
                        _, select_index = torch.topk(confidence[0], k=int(k_this.item()))
                        transfer_index[0, select_index] = True

                    # Apply finalized tokens
                    x_embeds[transfer_index] = x0_embeds[transfer_index]
                    x_tokens[transfer_index]  = x0[transfer_index]

                    # Stop sequence check (only when stop_tokens is non-empty)
                    if stop_tokens:
                        generated_part = x_tokens[0, L_prompt:L_prompt + gen_length]
                        for stop_seq in stop_tokens:
                            if not stop_seq:
                                continue
                            for start_idx in range(generated_part.size(0) - len(stop_seq) + 1):
                                if torch.all(
                                    ...
                                    == torch.tensor(stop_seq, device=x_tokens.device)
                                ):
                                    current_position = L_prompt + start_idx
                                    if not found_stop_seq or current_position < stop_position:
                                        stop_position = current_position
                                        found_stop_seq = True
                                    break
            ...
            # ===== Return cont only (token ids) =====
            if found_stop_seq:
                cont = x_tokens[:, L_prompt:stop_position]
            else:
                cont = x_tokens[:, L_prompt:L_prompt + gen_length]

            return cont.to(torch.long).detach().cpu()
